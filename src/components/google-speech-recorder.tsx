...
import { useGoogleSpeechTranscription } from "@/hooks/use-google-speech-transcription";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Mic, MicOff, Trash2 } from "lucide-react";

interface GoogleSpeechRecorderProps {
  onTranscriptChange?: (transcript: string) => void;
}

export function GoogleSpeechRecorder({
  onTranscriptChange,
}: GoogleSpeechRecorderProps) {
  const {
    state,
    transcript,
    interimTranscript,
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>Recording (Google Speech)</span>
