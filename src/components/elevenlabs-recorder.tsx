...
interface ElevenLabsRecorderProps {
  onTranscriptChange?: (transcript: string) => void;
}

export function ElevenLabsRecorder({
  onTranscriptChange,
}: ElevenLabsRecorderProps) {
  const {
    state,
    transcript,
    interimTranscript,
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>Recording (ElevenLabs)</span>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          ElevenLabs Speech-to-Text API を使用
        </p>
      </CardContent>
    </Card>
  );
